...
const MyQuotes = props => {
	...
	props.quotes.map( q => <div style={{"display": "auto-fill"}}><Link to={`/my-quotes/${q.id}`} className="link" key={q.id}>{<QuickQuoteCard quote={q}/>}<br/></Link></div> ): "No quotes at the moment"
	return quoteCards
	}
